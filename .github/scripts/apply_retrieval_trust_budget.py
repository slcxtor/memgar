...
def add_after(path: str, marker: str, addition: str) -> None:
    content = read(path)
...
def add_before(path: str, marker: str, addition: str) -> None:
    content = read(path)
    if addition.strip() in content:
        return
    if marker not in content:
...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if old in content:
        ...
        return
    if new.strip() not in content:
        raise RuntimeError(f"missing replace target in {path}: {old!r}")
...
secure_store = read("memgar/secure_memory_store.py")
if "_apply_retrieval_trust_budget" not in secure_store:
    secure_store, count = re.subn(
        r"    def guard_retrieval\([\s\S]*?\n    def guard_tool_result\(",
        new_guard + "\n    def guard_tool_result(",
        secure_store,
        count=1,
    )
    if count != 1:
        raise RuntimeError("guard_retrieval replacement failed")
    write("memgar/secure_memory_store.py", secure_store)
...
readme_marker = "if tool_verdict.allowed:\n    use_tool_output(tool_verdict.safe_content)\n```\n"
if readme_marker in read("README.md"):
    add_after(
        "README.md",
        readme_marker,
        textwrap.dedent(
