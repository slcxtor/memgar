...
class EUAIActReport:
    ...
    def generate_markdown(self) -> str:
        ...
        if gap["critical_gaps"]:
            lines += ["### ❌ Critical Gaps (High Priority)", ""]
            for g in gap["critical_gaps"]:
                lines.append(f"- {g}")
            lines.append("")

        lines += ["---", "", "## Article-by-Article Compliance Checks", ""]
        ...
        lines += [
            "---",
            "",
            "## Article 49 — EU Database Registration Data",
            "",
            "| Field | Value |",
            "|-------|-------|",
        ]
        ...
        lines += [
            ...
            "## Disclaimer",
            "",
            "This report is generated automatically by Memgar and constitutes a",
            "technical self-assessment. It does not constitute legal advice and does",
            "not replace a formal conformity assessment by a qualified notified body.",
            "Organizations should consult qualified legal and compliance professionals",
            "before submitting to regulatory authorities.",
            "",
